...
@keyframes float {{
  ...
  50%      {{ transform: rotateX(60deg) rotateZ(-15deg) translateY(-18px); }}
}}
...
<!-- Onda de pulso: FUERA de cualquier parent con transform -->
...
<button class="btn-egg" id="btn">
  <svg class="btn-hand" viewBox="-2 2.5 13 12.5" xmlns="http://www.w3.org/2000/svg">
    ...
    <!--
      Dedos juntos, mismo ancho (2u), -1u mas cortos:
      D1 ind : x1-3, top y=4  (height=2)
      D2 med : x3-5, top y=3  (height=3, mas alto)
      D3 anu : x5-7, top y=4  (height=2, igual D1)
      D4 men : x7-9, top y=5  (height=1, mas corto)
      Palma  : x1-9, y6-14
      Pulgar : x-1..1, y7-11
      Lineas entre dedos: x=3, x=5, x=7
    -->

    <!-- sombra 3D -->
    <path fill="#0369a1" transform="translate(0.6,0.6)"
      d="M1,4 H3 V3 H5 V4 H7 V5 H9 V14 H1 V11 H-1 V7 H1 Z"/>
    ...
    <!-- interior blanco -->
    <path fill="white"
      d="M1.4,4.4 H2.6 V3.4 H4.6 V4.4 H6.6 V5.4 H8.6 V13.6
    ...
    <rect x="3.4" y="3.4" width="1.2" height="0.4" fill="#e0f7ff" opacity="0.85"/>
    <rect x="5.4" y="4.4" width="1.2" height="0.4" fill="#e0f7ff" opacity="0.85"/>
    <rect x="7.4" y="5.4" width="1.2" height="0.4" fill="#e0f7ff" opacity="0.85"/>
    <!-- highlight top pulgar -->
    <rect x="-0.6" y="7.4" width="1.2" height="0.4" fill="#e0f7ff" opacity="0.85"/>
    <!-- highlight borde izquierdo -->
    <rect x="1.4"  y="4.4" width="0.28" height="9.2" fill="#e0f7ff" opacity="0.4"/>
    <rect x="-0.6" y="7.4" width="0.28" height="3.2" fill="#e0f7ff" opacity="0.5"/>
    <rect x="1.4"  y="6"   width="7.2" height="0.28" fill="#e0f7ff" opacity="0.4"/>
  </svg>
...
(function() {{
  ...
  function makePoints() {{
    ...
    /* ---- Capa 1: zigzag angular — posicion+altura varían levemente por click ---- */
    /* Cada keyframe tiene un offset aleatorio pequeño bakeado al inicio del call */
    const baseKeys = [
      [0.00,  0.00, 0,    0   ],  /* [t, dy, dt_range, ddy_range] */
      [0.06, -0.10, 0.02, 0.05],
      [0.12,  0.18, 0.02, 0.06],
      [0.18, -0.22, 0.02, 0.07],
      [0.23,  0.14, 0.02, 0.06],
      ...
      [0.37, -0.05, 0.01, 0.04],
      [0.40,  0.28, 0.02, 0.07],
      [0.46, -0.60, 0.02, 0.08],  /* picos raros */
      [0.52,  0.65, 0.02, 0.08],
      [0.57, -0.56, 0.02, 0.07],
      [0.63,  0.60, 0.02, 0.08],
      [0.68, -0.52, 0.02, 0.07],
    ...
    /* Bake los offsets UNA vez por click */
    const keys = baseKeys.map(([t, dy, dt, ddy]) => [
      t  + (r() - 0.5) * dt,
      dy + (r() - 0.5) * ddy,
    ]);
    /* Garantizar orden creciente en t (los offsets son pequeños pero por si acaso) */
    ...
    for (let x = 0; x <= W; x += 2) {{
      ...
      const spike   = gauss(x, spikeCX, W * 0.016, -spikeAmp);
      const sDip    = gauss(x, dipCX,   W * 0.013,  canvas.height * 0.20);
      const pBump   = gauss(x, pBumpCX, W * 0.022, -canvas.height * 0.09);
      const jitter  = (r() - 0.5) * canvas.height * 0.022 * env;
  ...
  function drawWave(pts, alpha) {{
    ...
    /* glow capas */
    ["{glow2}", "{glow1}"].forEach((col, i) => {{
      ctx.save();
      ctx.beginPath();
      ctx.moveTo(pts[0][0], pts[0][1]);
      for (let k = 1; k < pts.length; k++) ctx.lineTo(pts[k][0], pts[k][1]);
      ctx.strokeStyle = col;
      ctx.lineWidth   = i === 0 ? 18 : 8;
      ctx.globalAlpha = alpha * (i === 0 ? 0.25 : 0.5);
      ctx.shadowColor = "{glow1}";
      ctx.shadowBlur  = i === 0 ? 30 : 12;
      ctx.stroke();
      ctx.restore();
    }});
    /* linea principal negra */
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(pts[0][0], pts[0][1]);
    for (let k = 1; k < pts.length; k++) ctx.lineTo(pts[k][0], pts[k][1]);
  ...
  function triggerWave() {{
    ...
    function frame(ts) {{
      const t = Math.min((ts - start) / TOTAL, 1);
      let alpha;
