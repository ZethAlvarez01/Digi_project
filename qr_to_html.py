...
<svg id="pulse-wave" viewBox="0 0 1000 260" preserveAspectRatio="none"
  ...
  <path id="wave-path" fill="none" stroke="#000000" stroke-width="4"
...
(function() {{
  ...
  function makeWave() {{
    ...
    for (let x = 0; x <= W; x += 2) {{
      ...
      /* ruido de fondo solo en flancos, casi nulo en spike */
      const distMid = Math.abs(x - MID);
      const spikeMask = 1 - Math.exp(-0.5 * Math.pow(distMid / 80, 2));
      const noise    = (Math.random() - 0.5) * 10 * env * spikeMask;
      const baseline = Math.sin(x * 0.025) * 6 * env;
      /* componentes ECG — Y negativo = sube en pantalla */
      const pWave = gauss(x, MID - 150, 28, -18);
      const rWave = gauss(x, MID,        16, -148);
      const sWave = gauss(x, MID + 28,   14,  38);
      const tWave = gauss(x, MID + 130,  42, -42);
      const y = CY + baseline + noise + pWave + rWave + sWave + tWave;
