...
<svg id="pulse-wave" viewBox="0 0 1000 100" preserveAspectRatio="none"
  ...
  <defs>
    <filter id="wglow" x="-5%" y="-200%" width="110%" height="500%">
  ...
  <path id="wave-path" fill="none" stroke="#000000" stroke-width="1.2"
...
(function() {{
  ...
  const W = 1000, CY = 50;  /* CY=50/100 = 50vh exacto */
  let   animId = null;
  ...
  function makeWave() {{
    ...
    for (let x = 0; x <= W; x += 2) {{
      ...
      const noise    = (Math.random() - 0.5) * 3 * env * spikeMask;
      const baseline = Math.sin(x * 0.025) * 2 * env;
      const pWave = gauss(x, MID - 150, 28, -5);
      const rWave = gauss(x, MID,        16, -32);
      const sWave = gauss(x, MID + 28,   14,  10);
      const tWave = gauss(x, MID + 130,  42, -10);
